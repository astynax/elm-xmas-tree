...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var elemLen = function (el) {    var _p0 = el;if (_p0.ctor === "Fur") {    return _p0._1;} else {    return 1;}};
   var forever = function (x) {    return A2($Random.map,function (_p1) {    return x;},$Random.bool);};
   var oneOf = function (xs) {
      return A3($Basics.flip,
      $Random.map,
      A2($Random.$int,0,$List.length(xs) - 1),
      function (n) {
         var _p2 = A2($List.drop,n,xs);
         if (_p2.ctor === "::") {
               return _p2._0;
            } else {
               return _U.crashCase("Main",{start: {line: 110,column: 5},end: {line: 112,column: 38}},_p2)("Oops!");
            }
      });
   };
   var renderElem = function (el) {
      return A2($Html.span,
      _U.list([$Html$Attributes.$class(A2($String.append,
      "decoration ",
      function () {
         var _p4 = el;
         switch (_p4.ctor)
         {case "Star": return "star";
            case "Toy1": return "toy1";
            case "Toy2": return "toy2";
            case "Toy3": return "toy3";
            default: return "fur";}
      }()))]),
      _U.list([$Html.text(function () {
         ...
         _v3_10: do {
            ...
            {case "Star": return "*";
               ...
               case "Toy2": return "O";
               case "Toy3": return "@";
               default: if (_p5._0 === false) {
                       switch (_p5._1)
                       {case 1: return ">";
                          case 2: return ">>";
                          case 3: return ">>>";
                          default: break _v3_10;}
                    } else {
                       switch (_p5._1)
                       {case 1: return "<";
                          case 2: return "<<";
         ...
         return $Basics.toString(el);
      }())]));
   };
   var render = function (rs) {
      return A2($Html.div,
      _U.list([$Html$Attributes.id("pine")]),
      A2($List.map,
      function (_p6) {
         var _p7 = _p6;
         return A2($Html.div,
         _U.list([]),
         A2($List._op["::"],A2($Html.span,_U.list([]),_U.list([$Html.text(A2($String.repeat,_p7._0," "))])),A2($List.map,renderElem,_p7._1)));
      },
      rs));
   };
   var Star = {ctor: "Star"};
   ...
   var Toy2 = {ctor: "Toy2"};
   var Toy1 = {ctor: "Toy1"};
   var toy = oneOf(_U.list([Toy1,Toy2,Toy3,Star]));
   var Fur = F2(function (a,b) {    return {ctor: "Fur",_0: a,_1: b};});
   var fur = A3($Random.map2,Fur,$Random.bool,A2($Random.$int,1,3));
   var chain = F2(function (n,isToy) {
      return _U.cmp(n,5) > 0 ? A2($Random.andThen,
      isToy ? toy : fur,
      function (el) {
         var n$ = n - elemLen(el);
         return A2($Random.map,F2(function (x,y) {    return A2($List._op["::"],x,y);})(el),A2(chain,n$,$Basics.not(isToy)));
      }) : isToy ? _U.cmp(n,4) < 1 ? A2($Random.map,function (t) {    return _U.list([t,A2(Fur,true,n - 1)]);},toy) : A4($Random.map3,
      F3(function (t1,f,t2) {    return _U.list([t1,f,t2,A2(Fur,true,3 - elemLen(f))]);}),
      toy,
      A3($Random.map2,Fur,$Random.bool,A2($Random.$int,1,2)),
      toy) : A2($Random.andThen,
      fur,
      function (f) {
         return A2($Random.map,F2(function (x,y) {    return A2($List._op["::"],x,y);})(f),A2(chain,n - elemLen(f),true));
      });
   });
   var fixDirection = function (es) {
      var _p8 = es;
      if (_p8.ctor === "::" && _p8._0.ctor === "Fur") {
            return A2($List._op["::"],A2(Fur,false,_p8._0._1),_p8._1);
         } else {
            return es;
         }
   };
   var row = function (n) {
      var _p9 = n;
      switch (_p9)
      {case 1: return forever(_U.list([Star]));
         case 2: return A2($Random.map,function (x) {    return _U.list([A2(Fur,false,1),x,A2(Fur,true,1)]);},toy);
         case 3: return A2($Random.map,function (x) {    return _U.list([A2(Fur,false,2),x,A2(Fur,true,2)]);},toy);
         default: return A2($Random.map,fixDirection,A2(chain,n * 2 - 1,false));}
   };
   var pine = function (n) {
      var go = function (cnt) {
         return _U.cmp(cnt,n) > 0 ? forever(_U.list([])) : A2($Random.andThen,
         row(cnt),
         function (r) {
            return A2($Random.map,F2(function (x,y) {    return A2($List._op["::"],x,y);})({ctor: "_Tuple2",_0: n - cnt,_1: r}),go(cnt + 1));
         });
      };
      return go(1);
   };
   var pineView = function (height) {
      return function (_p10) {
         return render($Basics.fst(A2($Random.generate,pine(height),$Random.initialSeed($Basics.round($Time.inSeconds(_p10))))));
      };
   };
   var view = function (t) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.div,_U.list([$Html$Attributes.id("happynewyear")]),_U.list([$Html.text("Happy New Year!")]))
              ,A2($Html.div,_U.list([$Html$Attributes.id("pine-container")]),_U.list([A2(pineView,25,t)]))
              ,A2($Html.div,
